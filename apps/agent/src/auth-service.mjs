...
function parseCookies(header) {
  ...
  for (const part of header.split(";")) {
    ...
    if (key) {
      try { cookies.set(key, decodeURIComponent(value)); } catch { /* ignore malformed cookie */ }
    }
  }
  return cookies;
}

function cookieHeader(name, value, { maxAgeSeconds, secure = false, httpOnly = true } = {}) {
...
async function requestJson(fetchImpl, url, init) {
  ...
  if (!response.ok) throw fail(typeof payload?.message === "string" ? payload.message : `GitHub request failed (${response.status})`, response.status === 401 ? "GITHUB_UNAUTHORIZED" : "GITHUB_AUTH_FAILED", response.status >= 500 ? 502 : 502);
  return payload;
}

function safeHeaderToken(value) {
  if (typeof value !== "string" || value.length < 32 || value.length > 256 || !/^[A-Za-z0-9_-]+$/.test(value)) throw fail("CSRF token is missing or invalid", "CSRF_REQUIRED", 403);
  return value;
}

function constantTimeEqual(left, right) {
  try {
    const a = Buffer.from(safeHeaderToken(left));
    const b = Buffer.from(safeHeaderToken(right));
...
export class GitHubAuthService {
  ...
  constructor({
    ...
    csrfCookieName = process.env.PINAKA_CSRF_COOKIE || "pinaka_csrf",
  ...
  begin() {
    ...
    const params = new URLSearchParams({ client_id: this.#clientId, redirect_uri: this.#redirectUri, state, scope: this.#scope, code_challenge: makeChallenge(verifier), code_challenge_method: "S256", allow_signup: "true" });
  ...
  async callback({ code, state } = {}) {
    ...
    const tokenPayload = await requestJson(this.#fetch, GITHUB_TOKEN_URL, { method: "POST", headers: { accept: "application/json", "content-type": "application/json" }, body: JSON.stringify({ client_id: this.#clientId, client_secret: this.#clientSecret, code: code.trim(), redirect_uri: this.#redirectUri, code_verifier: pending.verifier }) });
    ...
    const user = await requestJson(this.#fetch, GITHUB_USER_URL, { headers: { accept: "application/vnd.github+json", authorization: `Bearer ${tokenPayload.access_token}`, "x-github-api-version": "2022-11-28" } });
    ...
    const session = {
      ...
      user: { id: user.id, login: user.login.trim(), name: typeof user.name === "string" ? user.name.trim() : null, avatarUrl: typeof user.avatar_url === "string" ? user.avatar_url : null },
      createdAt: now,
    ...
    return {
      ...
      setCookies: [
        ...
        cookieHeader(this.#csrfCookieName, csrfToken, { maxAgeSeconds, secure: this.#secureCookies, httpOnly: false })
      ]
    };
  }
  ...
  getSession(requestHeaders) {
    ...
    if (session.expiresAt <= this.#now()) { this.#sessions.delete(sessionId); return null; }
    return { id: session.id, user: { ...session.user }, githubToken: session.githubToken, csrfToken: session.csrfToken };
  }

  assertCsrf(requestHeaders) {
    const session = this.getSession(requestHeaders);
    if (!session) throw fail("GitHub sign-in is required", "AUTH_REQUIRED", 401);
    const supplied = requestHeaders?.["x-csrf-token"] || requestHeaders?.["X-CSRF-Token"];
    if (!constantTimeEqual(supplied, session.csrfToken)) throw fail("CSRF token is missing or invalid", "CSRF_REQUIRED", 403);
    return session;
  }
  ...
  logout(requestHeaders) {
    ...
    const expired = { maxAgeSeconds: 0, secure: this.#secureCookies };
    return [cookieHeader(this.#cookieName, "", { ...expired, httpOnly: true }), cookieHeader(this.#csrfCookieName, "", { ...expired, httpOnly: false })];
  }
