...
function makeJob(taskId, repositoryUrl, task) {
  return { id: taskId, repositoryUrl, task, status: "queued", stage: "queued", message: "Waiting to start", createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), result: null, error: null };
}
...
export class AgentTaskRunner {
  ...
  #subscribers = new Map();
  #githubTokens = new Map();
  #routerFactory;
  #gitFactory;
  #registryFactory;
  #agentRunner;
  #githubToken;
  constructor({ workspaceRoot, workspaceManager, routerFactory = buildDefaultRouter, gitFactory = ({ workspaceRoot: root }) => new GitRepository({ workspaceRoot: root }), registryFactory = ({ workspaceRoot: root, githubToken, onToolEvent }) => createToolRegistry({ workspaceRoot: root, githubToken, onToolEvent }), agentRunner = runAutonomousRepairLoop, githubToken = "" } = {}) {
    this.#workspaceManager = workspaceManager || new WorkspaceManager({ rootDirectory: workspaceRoot });
    this.#routerFactory = routerFactory; this.#gitFactory = gitFactory; this.#registryFactory = registryFactory; this.#agentRunner = agentRunner; this.#githubToken = typeof githubToken === "string" ? githubToken.trim() : "";
  }
  ...
  subscribe(taskId, listener) {
    validateTaskId(taskId); if (!this.#jobs.has(taskId)) throw Object.assign(new Error("task not found"), { statusCode: 404, code: "TASK_NOT_FOUND" }); if (typeof listener !== "function") throw new TypeError("event listener must be a function");
    let listeners = this.#subscribers.get(taskId); if (!listeners) { listeners = new Set(); this.#subscribers.set(taskId, listeners); } listeners.add(listener);
  ...
  async start({ repositoryUrl, task, taskId = crypto.randomUUID().replaceAll("-", "").slice(0, 20), model, apiKey, baseUrl, githubToken = "" } = {}) {
    const safeRepositoryUrl = validateRepositoryUrl(repositoryUrl); const safeTask = validateText(task, "task", MAX_TASK_CHARS); validateTaskId(taskId);
    ...
    const job = makeJob(taskId, safeRepositoryUrl, safeTask); this.#jobs.set(taskId, job); this.#eventHistory.set(taskId, []); if (token) this.#githubTokens.set(taskId, token);
    this.#emit(job, "queued", "Task queued"); void this.#run(job, { model, apiKey, baseUrl, githubToken: token }); return { ...job };
  }
  #emit(job, type, message, data = {}) {
    ...
    const history = this.#eventHistory.get(job.id) || []; if (history.length >= MAX_EVENTS_PER_TASK) history.shift(); history.push(event); this.#eventHistory.set(job.id, history);
    for (const listener of this.#subscribers.get(job.id) || []) { try { listener(event); } catch {} }
  }
  ...
  async #run(job, modelOptions) {
    ...
    try {
      ...
      await git.clone(job.repositoryUrl, { githubToken: modelOptions.githubToken || "" });
      ...
      const registry = this.#registryFactory({ workspaceRoot: workspace.path, githubToken: modelOptions.githubToken || "", onToolEvent: (event) => { const safeEvent = sanitizeToolEvent(event); if (!safeEvent) return; const label = safeEvent.type === "tool.start" ? `Running ${safeEvent.tool}` : safeEvent.ok ? `Finished ${safeEvent.tool}` : `Failed ${safeEvent.tool}`; this.#emit(job, safeEvent.type, label, safeEvent); } });
      const router = this.#routerFactory({ model: modelOptions.model, apiKey: modelOptions.apiKey, baseUrl: modelOptions.baseUrl });
      ...
      let diff = null;
      try { const diffResult = await git.diff({ staged: false, maxOutputBytes: MAX_RETAINED_DIFF_CHARS }); if (diffResult && typeof diffResult === "object") { const text = typeof diffResult.text === "string" ? diffResult.text : ""; diff = { text, truncated: diffResult.truncated === true, changeCount: Number.isInteger(result?.finalReview?.diff?.changeCount) ? result.finalReview.diff.changeCount : null }; } }
      catch (error) { this.#emit(job, "diff.error", "Unable to capture the final diff", { errorCode: typeof error?.code === "string" ? error.code.slice(0, 128) : "DIFF_CAPTURE_FAILED" }); }
      const finalResult = diff ? { ...result, diff } : result;
      this.#update(job, { status: ["passed", "repaired", "accepted"].includes(result.status) ? "completed" : "needs_attention", stage: "complete", message: result.status === "repaired" ? "Task repaired and verified" : ["passed", "accepted"].includes(result.status) ? "Task verified" : "Task finished without full verification", result: finalResult });
    } catch (error) {
      ...
      this.#githubTokens.delete(job.id);
      if (workspace) { try { await this.#workspaceManager.release(job.id); } catch { try { await this.#workspaceManager.discard(job.id); } catch {} } }
    }
  }
}
