...
function sameOrigin(req) {
  const configured = String(process.env.PINAKA_PUBLIC_ORIGIN || "").trim().replace(/\/$/, "");
  ...
  if (origin && origin !== expected) throw Object.assign(new Error("request origin is not allowed"), { statusCode: 403, code: "CSRF_ORIGIN_REJECTED" });
  if (!origin) { const referer = String(req.headers.referer || ""); if (!referer.startsWith(`${expected}/`)) throw Object.assign(new Error("request origin could not be verified"), { statusCode: 403, code: "CSRF_ORIGIN_REQUIRED" }); }
}
...
function track(job) { void checkpoint(job.id); let off = null; try { off = taskRunner.subscribe(job.id, (event) => { void checkpoint(job.id); if (TERMINAL.has(event.status)) { try { off?.(); } catch {} } }); } catch {} }
function writeSse(res, event) { res.write(`id: ${event.id}\nevent: task\ndata: ${JSON.stringify(event)}\n\n`); }
function stream(res, id) {
  let history; let live = true;
  try { history = taskRunner.events(id); } catch { live = false; history = getTask(id).events || []; }
  ...
  res.flushHeaders?.(); res.write(": connected\n\n"); history.forEach((event) => writeSse(res, event));
  if (!live || !history.at(-1) || TERMINAL.has(history.at(-1).status)) { res.end(); return; }
  let closed = false; let heartbeat = setInterval(() => { if (!closed) res.write(": heartbeat\n\n"); }, 15000);
  const off = taskRunner.subscribe(id, (event) => { if (closed) return; writeSse(res, event); if (TERMINAL.has(event.status)) { closed = true; clearInterval(heartbeat); off(); res.end(); } });
  res.on("close", () => { closed = true; clearInterval(heartbeat); off(); });
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/auth/github") { res.writeHead(302, { location: authService.begin(), "cache-control": "no-store" }); return res.end(); }
    if (req.method === "GET" && url.pathname === "/auth/github/callback") { const result = await authService.callback({ code: url.searchParams.get("code") || "", state: url.searchParams.get("state") || "" }); res.writeHead(302, { location: "/?auth=success", "set-cookie": result.setCookies, "cache-control": "no-store" }); return res.end(); }
    ...
    if (req.method === "POST" && url.pathname === "/v1/agent/run") { const user = mutationSession(req); const body = await readJson(req); const job = await taskRunner.start({ ...body, ownerId: user.user.id, githubToken: user.githubToken }); await taskStore.save({ ...job, events: taskRunner.events(job.id) }); track(job); return json(res, 202, decorated(job)); }
  ...
  } catch (error) { const status = Number.isInteger(error?.statusCode) ? error.statusCode : 400; return json(res, status, { error: status >= 500 ? "internal_error" : "invalid_request", message: error?.message || "request failed", code: error?.code || null }); }
});
