...
function sendJson(res, statusCode, payload, headers = {}) {
  ...
  res.writeHead(statusCode, { "content-type": "application/json; charset=utf-8", "content-length": Buffer.byteLength(body), "cache-control": "no-store", "access-control-allow-origin": "*", "access-control-allow-headers": "content-type", "access-control-allow-methods": "GET,POST,OPTIONS", ...headers });
  res.end(body);
}
async function readJson(req) {
  let size = 0; const chunks = [];
  for await (const chunk of req) { size += chunk.length; if (size > MAX_BODY_BYTES) throw Object.assign(new Error("request body is too large"), { statusCode: 413 }); chunks.push(chunk); }
  if (size === 0) return {};
  try { return JSON.parse(Buffer.concat(chunks).toString("utf8")); } catch { throw Object.assign(new Error("request body must contain valid JSON"), { statusCode: 400 }); }
}
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.method === "GET" && url.pathname === "/auth/github/callback") {
      ...
      res.writeHead(302, { location: "/?auth=success", "set-cookie": result.setCookie, "cache-control": "no-store" }); res.end(); return;
    }
    ...
    if (req.method === "POST" && url.pathname === "/v1/auth/logout") { sendJson(res, 200, { authenticated: false }, { "set-cookie": authService.logout(req.headers) }); return; }
    ...
    if (req.method === "POST" && url.pathname === "/v1/agent/run") {
      const body = await readJson(req); const session = authService.getSession(req.headers);
      if (!session) throw Object.assign(new Error("GitHub sign-in is required to run a repository task"), { statusCode: 401, code: "AUTH_REQUIRED" });
      const job = await taskRunner.start({ ...body, githubToken: session.githubToken }); sendJson(res, 202, approvalService.decorate(job)); return;
    }
    ...
    if (req.method === "POST") {
      ...
      if (approvalTaskId) {
        ...
        if (!session) throw Object.assign(new Error("GitHub sign-in is required for this action"), { statusCode: 401, code: "AUTH_REQUIRED" });
        const body = await readJson(req); const updated = await approvalService.decide(taskRunner.get(approvalTaskId), body.decision, { githubToken: session.githubToken }); sendJson(res, 200, updated); return;
      }
    }
    if (req.method === "GET") {
      const eventsTaskId = extractTaskEventsId(url.pathname); if (eventsTaskId) { streamTaskEvents(res, eventsTaskId); return; }
      const taskId = extractTaskId(url.pathname); if (taskId) { sendJson(res, 200, approvalService.decorate(taskRunner.get(taskId))); return; }
    }
