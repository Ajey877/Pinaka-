...
const server = http.createServer(async (req, res) => { let timeout = null; try {
  ...
  if (req.method === "POST" && url.pathname === "/v1/agent/run") {
    ...
    try { const body = await readJson(req); const job = await taskRunner.start({ ...body, ownerId: user.user.id, githubToken: user.githubToken }); await taskStore.save({ ...job, events: taskRunner.events(job.id) }); track(job, user.user.id); return json(res, 202, decorated(job)); }
...
} catch (error) { const status = Number.isInteger(error?.statusCode) ? error.statusCode : 400; return json(res, status, { error: status >= 500 ? "internal_error" : "invalid_request", message: error?.message || "request failed", code: error?.code || null, ...(status === 429 ? { retryAfterMs: HARDENING.rateWindowMs } : {}) }); } finally { if (timeout) clearTimeout(timeout); } });
