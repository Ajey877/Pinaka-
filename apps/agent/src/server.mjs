...
function extractApprovalId(pathname) {
  const match = pathname.match(/^\/v1\/agent\/tasks\/([A-Za-z0-9][A-Za-z0-9_-]{0,63})\/approval$/);
  return match?.[1] || null;
}
...
function streamTaskEvents(res, taskId) {
  ...
  if (lastEvent && lastEvent.status === "failed") { res.end(); return; }
  let closed = false;
  let heartbeat = null;
  const unsubscribe = taskRunner.subscribe(taskId, (event) => {
    if (closed) return;
    writeSse(res, event);
    if (event.status === "failed") { closed = true; if (heartbeat) clearInterval(heartbeat); unsubscribe(); res.end(); }
  });
  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (approvalTaskId) {
        ...
        const updated = await approvalService.decide(taskRunner.get(approvalTaskId), body.decision);
