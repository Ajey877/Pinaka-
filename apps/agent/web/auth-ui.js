...
style.textContent = `.auth-area{display:flex;align-items:center;gap:8px}.github-button,.auth-user,.auth-logout{border:1px solid var(--hairline);border-radius:14px;background:var(--card-strong);color:var(--text);font:inherit;font-weight:700}.github-button{padding:9px 13px;text-decoration:none}.auth-user{display:flex;align-items:center;gap:8px;padding:5px 9px}.auth-user img{width:24px;height:24px;border-radius:50%;object-fit:cover}.auth-user span{max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.auth-logout{padding:8px 10px;cursor:pointer}.github-button:hover,.auth-logout:hover{transform:translateY(-1px)}@media(max-width:640px){.auth-user span{display:none}.github-button{padding:8px 10px}.auth-logout{padding:8px}}`;
...
async function loadAuth() {
  try {
    ...
    if (response.ok && data.authenticated === true) showSignedIn(data.user);
    else showSignedOut();
  } catch {
    showSignedOut();
  }
